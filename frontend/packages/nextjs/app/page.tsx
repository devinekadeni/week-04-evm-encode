...
function ProposalList() {
  const [proposals, setProposals] = useState<{ name: string; votesCount: number }[]>([]);

  useEffect(() => {
    fetch("http://localhost:3001/proposals")
      .then(res => res.json())
      .then(data => setProposals(data.result));
  }, []);

  return (
    <div className="card w-96 bg-primary text-primary-content mt-4">
      <div className="card-body">
        <h2 className="card-title">Current Proposal Votes</h2>
        {proposals.length ? (
          <div className="flex gap-2 align-middle justify-between">
            {proposals.map(proposal => (
              <div key={proposal.name} className="flex-col text-center gap-2">
                <p className="font-bold">{proposal.name}</p>
                <p>{proposal.votesCount} Vote(s)</p>
              </div>
            ))}
          </div>
        ) : (
